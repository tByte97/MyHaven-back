...
python manage.py test telegram_api.tests # Telegram API тести
...
- `users.services` (Export/Deletion) → [users/tests.py](users/tests.py)
- Telegram API endpoints → [telegram_api/tests.py](telegram_api/tests.py)

### Запуск

```bash
python manage.py test users.tests telegram_api.tests
```
...
- Схема генерується через `drf-spectacular`.
- Потрібен активний сервер: `python manage.py runserver`.

---

## 🧾 Звіт та інструкція по задачах

### Звіт виконання

- Додані тести для сервісів експорту та видалення акаунта.
- Додані тести для Telegram API (health, dashboard, categories, budgets, transactions).
- Підключено OpenAPI схему та Swagger UI.

### Інструкція

1. Встановити залежності: `pip install -r requirements.txt`
2. Запустити тести: `python manage.py test users.tests telegram_api.tests`
3. Підняти сервер: `python manage.py runserver`
4. Відкрити Swagger UI: `http://localhost:8000/api/docs/`
